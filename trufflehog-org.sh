...
docker run --rm -it --log-driver=none -a stdin -a stdout -a stderr truffps aux|gdocker run --rm -it --log-driver=none -a stdin -a stdout -a stderr trufflesecurity/trufflehog:latest github --org=$org --issue-comments --pr-comments 2>&1 >> /tmp/tmp-trufflehog
...
grep -i "Detector Type:" /tmp/tmp-trufflehog |sort -u